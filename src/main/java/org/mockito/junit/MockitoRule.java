...
/**
 * Mockito JUnit Rule helps keeping tests clean. It initializes mocks, validates usage and detects
 * incorrect stubbing. Make sure to configure your rule with {@link #strictness(Strictness)} which
 * automatically detects <strong>stubbing argument mismatches</strong> and is planned to be the
 * default in Mockito v3.
 *
 * <p>Since Mockito 2.1.0, JUnit rule emits stubbing warnings and hints to System output (see {@link
 * MockitoHint}). The JUnit rule can be used instead of {@link MockitoJUnitRunner}. It requires
 * JUnit at least 4.7.
 *
 * <p>The rule adds following behavior:
 *
 * <ul>
 *   <li>Since 2.1.0, stubbing warnings and hints are printed to System output. Hints contain
 *       clickable links that take you right to the line of code that contains a possible problem.
 *       <strong>Please</strong> give us feedback about the stubbing warnings of JUnit rules in the
 *       issue tracker (<a href="https://github.com/mockito/mockito/issues/384">issue 384</a>). It's
 *       a new feature of Mockito 2.1.0. It aims to help debugging tests. If you wish the previous
 *       behavior, see {@link MockitoRule#silent()}. However, we would really like to know why do
 *       you wish to silence the warnings! See also {@link MockitoHint}.
 *   <li>Initializes mocks annotated with {@link org.mockito.Mock}, so that explicit usage of {@link
 ...
 *   <li>It is highly recommended to use the rule with {@link #strictness(Strictness)} configured to
 *       {@link Strictness#STRICT_STUBS}. It drives cleaner tests and improves debugging experience.
 *       The only reason this feature is not turned on by default is because it would have been an
 *       incompatible change and Mockito strictly follows <a href="http://semver.org">semantic
 *       versioning</a>.
 * </ul>
 *
 * Example use:
 ...
 * </code></pre>
 *
 * If you would like to take advantage of Mockito JUnit rule features but you cannot use the rule
 * because, for example, you use TestNG, there is a solution! {@link MockitoSession} API is intended
 * to offer cleaner tests and improved debuggability to users that cannot use Mockito's built-in
 * JUnit support (runner or the rule).
 *
 * @since 1.10.17
 */
