    ...
    public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classfileBuffer) {
        if (classBeingRedefined == null || !mocked.contains(classBeingRedefined) && !flatMocked.contains(classBeingRedefined) || EXCLUDES.contains(classBeingRedefined)) {
            ...
            try {
                ...
                byteBuddy.// new ClassFileLocator.Compound(
                redefine(// new ClassFileLocator.Compound(
                classBeingRedefined, // Note: The VM erases parameter meta data from the provided class file
                ClassFileLocator.Simple.of(classBeingRedefined.getName(), classfileBuffer)).// (bug). We just add this information manually.
                visit(new ParameterWritingVisitorWrapper(classBeingRedefined)).visit(mockTransformer).make().getBytes();
