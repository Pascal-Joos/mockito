...
abstract class ModuleHandler {
  ...
  private static class ModuleSystemFound extends ModuleHandler {
    ...
    boolean isExported(Class<?> source) {
      ...
      return (Boolean)
          invoke(isExportedUnqualified, invoke(getModule, source), source.getPackage().getName());
    }
    ...
    void adjustModuleGraph(Class<?> source, Class<?> target, boolean export, boolean read) {
      ...
      if (targetVisible) {
        ...
        try {
          intermediate =
              byteBuddy
                  ...
                  .name(
                      String.format(
                          "%s$%d",
                          "org.mockito.codegen.MockitoTypeCarrier", Math.abs(random.nextInt())))
                  .defineField("mockitoType", Class.class, Visibility.PUBLIC, Ownership.STATIC)
                  .make()
                  .load(
                      source.getClassLoader(),
                      loader.resolveStrategy(source, source.getClassLoader(), false))
                  .getLoaded();
          ...
          throw new MockitoException(
              join(
                  "Could not create a carrier for making the Mockito type visible to " + source,
                  "",
        ...
        implementation =
            MethodCall.invoke(getModule)
                .onMethodCall(MethodCall.invoke(forName).with(intermediate.getName()));
      }
        ...
        Class.forName(
            byteBuddy
                ...
                .name(
                    String.format(
                        "%s$%s$%d",
                        source.getName(), "MockitoModuleProbe", Math.abs(random.nextInt())))
