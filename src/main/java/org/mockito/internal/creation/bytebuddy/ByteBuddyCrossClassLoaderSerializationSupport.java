...
/**
 * This is responsible for serializing a mock, it is enabled if the mock is implementing {@link
 * Serializable}.
 *
 * <p>The way it works is to enable serialization with mode {@link
 * SerializableMode#ACROSS_CLASSLOADERS}, if the mock settings is set to be serializable the mock
 * engine will implement the {@link CrossClassLoaderSerializableMock} marker interface. This
 * interface defines a the {@link CrossClassLoaderSerializableMock#writeReplace()} whose signature
 * match the one that is looked by the standard Java serialization.
 *
 * <p>Then in the proxy class there will be a generated <code>writeReplace</code> that will delegate
 * to {@link ForWriteReplace#doWriteReplace(MockAccess)} of mockito, and in turn will delegate to
 * the custom implementation of this class {@link #writeReplace(Object)}. This method has a specific
 * knowledge on how to serialize a mockito mock that is based on ByteBuddy and will ignore other
 * Mockito MockMakers.
 *
 * <p><strong>Only one instance per mock! See {@link MockMethodInterceptor}</strong> TODO check the
 * class is mockable in the deserialization side
 *
...
class ByteBuddyCrossClassLoaderSerializationSupport implements Serializable {
   ...
   *
   * <p>Here's how it's working and why :
   *
   * <ol>
   *   <li>
   *       <p>When first entering in this method, it's because some is serializing the mock, with
   *       some code like :
   *       <pre class="code"><code class="java">
   * objectOutputStream.writeObject(mock);
   * </code></pre>
   *       <p>So, {@link ObjectOutputStream} will track the <code>writeReplace</code> method in the
   *       instance and execute it, which is wanted to replace the mock by another type that will
   *       encapsulate the actual mock. At this point, the code will return an {@link
   *       CrossClassLoaderSerializableMock}.
   *   <li>
   *       <p>Now, in the constructor {@link
   *       CrossClassLoaderSerializationProxy#CrossClassLoaderSerializationProxy(java.lang.Object)}
   *       the mock is being serialized in a custom way (using {@link
   *       MockitoMockObjectOutputStream}) to a byte array. So basically it means the code is
   *       performing double nested serialization of the passed <code>mockitoMock</code>.
   *       <p>However the <code>ObjectOutputStream</code> will still detect the custom <code>
   *       writeReplace</code> and execute it. <em>(For that matter disabling replacement via {@link
   *       ObjectOutputStream#enableReplaceObject(boolean)} doesn't disable the <code>writeReplace
   *       </code> call, but just just toggle replacement in the written stream, <strong><code>
   *       writeReplace</code> is always called by <code>ObjectOutputStream</code></strong>.)</em>
   *       <p>In order to avoid this recursion, obviously leading to a {@link StackOverflowError},
   *       this method is using a flag that marks the mock as already being replaced, and then
   *       shouldn't replace itself again. <strong>This flag is local to this class</strong>, which
   *       means the flag of this class unfortunately needs to be protected against concurrent
   *       access, hence the reentrant lock.
   * </ol>
   *
   * @param mockitoMock The Mockito mock to be serialized.
   * @return A wrapper ({@link CrossClassLoaderSerializationProxy}) to be serialized by the calling
   *     ObjectOutputStream.
   ...
   * <p>
   *
   * <p>When called in the constructor it will serialize the mock in a byte array using a custom
   * {@link MockitoMockObjectOutputStream} that will annotate the mock class in the stream. Other
   * information are used in this class in order to facilitate deserialization.
   *
   * <p>
   *
   * <p>Deserialization of the mock will be performed by the {@link #readResolve()} method via the
   * custom {@link MockitoMockObjectInputStream} that will be in charge of creating the mock class.
   */
  public static class CrossClassLoaderSerializationProxy implements Serializable {
     ...
     *
     * <p>Immediately serializes the Mockito mock using specifically crafted {@link
     * MockitoMockObjectOutputStream}, in a byte array.
     *
     ...
     * <p>
     *
     * <p>Uses the custom crafted {@link MockitoMockObjectInputStream} to deserialize the mock.
     *
   ...
   * <p>
   *
   * <p>This specifically crafted ObjectInoutStream has the most important role to resolve the
   * Mockito generated class. It is doing so via the {@link #resolveClass(ObjectStreamClass)} which
   * looks in the stream for a Mockito marker. If this marker is found it will try to resolve the
   * mockito class otherwise it delegates class resolution to the default super behavior. The mirror
   * method used for serializing the mock is {@link
   * MockitoMockObjectOutputStream#annotateClass(Class)}.
   *
   * <p>
   *
   * <p>When this marker is found, {@link ByteBuddyMockMaker#createMockType(MockCreationSettings)}
  ...
  public static class MockitoMockObjectInputStream extends ObjectInputStream {
     ...
     * <p>
     *
     * <p>Uses the fields {@link #typeToMock} and {@link #extraInterfaces} to create the Mockito
     * proxy class as the <code>ObjectStreamClass</code> doesn't carry useful information for this
     * purpose.
     *
    ...
    /**
     * Hack the <code>name</code> field of the given <code>ObjectStreamClass</code> with the <code>
     * newProxyClass</code>.
     *
     * <p>The parent ObjectInputStream will check the name of the class in the stream matches the
     * name of the one that is created in this method.
     *
     * <p>The CGLIB classes uses a hash of the classloader and/or maybe some other data that allow
     * them to be relatively unique in a JVM.
     *
     * <p>When names differ, which happens when the mock is deserialized in another ClassLoader, a
     * <code>java.io.InvalidObjectException</code> is thrown, so this part of the code is hacking
     * through the given <code>ObjectStreamClass</code> to change the name with the newly created
     * class.
     *
     ...
     * @return <code>true</code> if not marked as a Mockito, <code>false</code> if the class
     *     annotation marks a Mockito mock.
     */
   ...
   * <p>
   *
   * <p>This output stream has the role of marking in the stream the Mockito class. This marking
   * process is necessary to identify the proxy class that will need to be recreated.
   *
   * <p>The mirror method used for deserializing the mock is {@link
   ...
   *
   * <p>It will be applied before the creation of the mock when the mock setting says it should
   * serializable.
   */
