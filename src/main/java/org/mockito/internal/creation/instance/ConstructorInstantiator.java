...
public class ConstructorInstantiator implements Instantiator {
    ...
    private InstantiationException paramsException(Class<?> cls, Exception e) {
        return new InstantiationException(join("Unable to create instance of '" + cls.getSimpleName() + "'.", "Please ensure the target class has " + constructorArgsString() + " and executes cleanly."), e);
    }
    ...
    private InstantiationException noMatchingConstructor(Class<?> cls) {
        ...
        return new InstantiationException(join("Unable to create instance of '" + cls.getSimpleName() + "'.", "Please ensure that the target class has " + constructorString + outerInstanceHint + "."), null);
    }
    ...
    private static boolean paramsMatch(Class<?>[] types, Object[] params) {
        ...
        for (int i = 0; i < params.length; i++) {
            ...
            } else if ((!types[i].isPrimitive() && !types[i].isInstance(params[i])) || (types[i].isPrimitive() && !types[i].equals(Primitives.primitiveTypeOf(params[i].getClass())))) {
                return false;
            }
        }
        return true;
