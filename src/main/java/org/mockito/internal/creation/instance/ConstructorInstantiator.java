...
public class ConstructorInstantiator implements Instantiator {
   ...
   * This member is only used to for constructing error messages. If an outer inject exists, it
   * would be the first ([0]) element of the {@link #constructorArgs} array.
   */
    ...
    for (int i = 0; i < params.length; i++) {
      if (params[i] == null) {
        if (types[i].isPrimitive()) {
          ...
          || (types[i].isPrimitive()
              && !types[i].equals(Primitives.primitiveTypeOf(params[i].getClass())))) {
        return false;
      }
    }
    return true;
   ...
   * match than constructor Y to the given argument list if they are both applicable, constructor X
   * has at least one parameter than is more specific than the corresponding parameter of
   * constructor Y, and constructor Y has no parameter than is more specific than the corresponding
   * parameter in constructor X.
   *
   * <p>If {@code constructor} is a better match than the constructors in the {@code
   * matchingConstructors} list, the list is cleared, and it's added to the list as a singular best
   * matching constructor (so far).<br>
