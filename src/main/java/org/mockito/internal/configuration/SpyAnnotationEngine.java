 ...
 * <p>
 *
 * <p>Will try transform the field in a spy as with <code>Mockito.spy()</code>.
 *
 * <p>
 *
 * <p>If the field is not initialized, will try to initialize it, with a no-arg constructor.
 *
 * <p>
 *
 * <p>If the field is also annotated with the <strong>compatible</strong> &#64;InjectMocks then the
 * field will be ignored, The injection engine will handle this specific case.
 *
 * <p>
 *
...
public class SpyAnnotationEngine
  ...
  private static Constructor<?> noArgConstructorOf(Class<?> type) {
    ...
    try {
      ...
      throw new MockitoException(
          "Please ensure that the type '" + type.getSimpleName() + "' has a no-arg constructor.");
    }
    return constructor;
  }
  ...
  private static void assertNoIncompatibleAnnotations(
    ...
    for (Class<? extends Annotation> u : undesiredAnnotations) {
      if (field.isAnnotationPresent(u)) {
        throw unsupportedCombinationOfAnnotations(annotation.getSimpleName(), u.getSimpleName());
      }
    }
  }
}
