...
class DefaultMockitoPlugins implements MockitoPlugins {
  ...
  static {
    ...
    DEFAULT_PLUGINS.put(
        MockMaker.class.getName(), "org.mockito.internal.creation.bytebuddy.ByteBuddyMockMaker");
    DEFAULT_PLUGINS.put(
  ...
  public <T> T getDefaultPlugin(Class<T> pluginType) {
    if (pluginType == InstantiatorProvider.class) {
      ...
      return pluginType.cast(
          new InstantiatorProvider2Adapter(create(InstantiatorProvider2.class, className)));
    } else {
