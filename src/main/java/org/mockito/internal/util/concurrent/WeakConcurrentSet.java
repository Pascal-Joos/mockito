 ...
 * java.util.Set} interface because this implementation is incompatible with the set contract. While
 * iterating over a set's entries, any value that has not passed iteration is referenced non-weakly.
 */
public class WeakConcurrentSet<V> implements Runnable, Iterable<V> {
  ...
  /**
   * Determines the cleaning format. A reference is removed either by an explicitly started cleaner
   * thread associated with this instance ({@link Cleaner#THREAD}), as a result of interacting with
   * this thread local from any thread ({@link Cleaner#INLINE} or manually by submitting the
   * detached thread local to a thread ({@link Cleaner#MANUAL}).
   */
  public enum Cleaner {
    THREAD,
    INLINE,
