 ...
 *
 * <p>If the given field is already initialized, then <strong>the actual instance is
 * returned</strong>. This initializer doesn't work with inner classes, local classes, interfaces or
 * abstract types.
 */
public class FieldInitializer {
   ...
   *
   * <p>This constructor fail fast if the field type cannot be handled.
   *
  ...
  /**
   * Represents the strategy used to resolve actual instances to be given to a constructor given the
   * argument types.
   */
  public interface ConstructorArgumentResolver {
     ...
     *
     * <p>Checks on the real argument type or on the correct argument number will happen during the
     * field initialization {@link FieldInitializer#initialize()}. I.e the only responsibility of
     * this method, is to provide instances <strong>if possible</strong>.
     *
   ...
   *
   * <p>If a no-arg constructor can be found then the instance is created using this constructor.
  ...
  static class NoArgConstructorInstantiator implements ConstructorInstantiator {
    ...
    public FieldInitializationReport instantiate() {
      ...
      try {
        ...
        throw new MockitoException(
            "the type '" + field.getType().getSimpleName() + "' has no default constructor", e);
        ...
        throw new MockitoException(
            "InstantiationException (see the stack trace for cause): " + e.toString(), e);
      } catch (IllegalAccessException e) {
        throw new MockitoException(
            "IllegalAccessException (see the stack trace for cause): " + e.toString(), e);
      }
    }
  }
   ...
   *
   * <p>Choose the constructor with the highest number of parameters, then call the
   * ConstructorArgResolver to get actual argument instances. If the argResolver fail, then a
   * technical MockitoException is thrown is thrown. Otherwise the instance is created with the
   * resolved arguments.
   */
