...
/**
 * Optional Answer that can be used with {@link Mockito#mock(Class, Answer)}
 *
 ...
 * NullPointerException. This implementation of Answer returns SmartNulls instead of nulls.
 * SmartNull gives nicer exception message than NPE because it points out the line where unstubbed
 * method was called. You just click on the stack trace.
 *
 * <p>ReturnsSmartNulls first tries to return ordinary return values (see {@link
 * ReturnsMoreEmptyValues}) then it tries to return SmartNull. If the return type is not mockable
 * (e.g. final) then ordinary null is returned.
 *
 * <p>ReturnsSmartNulls will be probably the default return values strategy in Mockito 2.1.0
 */
