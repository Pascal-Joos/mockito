...
public class StackTraceFilter implements Serializable {
  ...
  /**
   * Example how the filter works (+/- means good/bad): [a+, b+, c-, d+, e+, f-, g+] -> [a+, b+, d+,
   * e+, g+] Basically removes all bad from the middle. <strike>If any good are in the middle of bad
   * those are also removed.</strike>
   */
   ...
   *
   * @param target The throwable target to find the first {@link StackTraceElement} that should not
   *     be filtered out per {@link StackTraceFilter#CLEANER}.
      ...
      while (true) {
        try {
          StackTraceElement stackTraceElement =
              (StackTraceElement) GET_STACK_TRACE_ELEMENT.invoke(JAVA_LANG_ACCESS, target, i);
