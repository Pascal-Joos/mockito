...
/**
 * Default implementation of stubbing lookup listener. Fails early if stub called with unexpected
 * arguments, but only if current strictness is set to STRICT_STUBS.
 */
class DefaultStubbingLookupListener implements StubbingLookupListener, Serializable {
  ...
  public void onStubbingLookup(StubbingLookupEvent event) {
    Strictness actualStrictness =
        determineStrictness(event.getStubbingFound(), event.getMockSettings(), currentStrictness);
