...
public interface OngoingStubbing<T> {
   ...
   * consecutive calls.
   *
   * <p>See examples in javadoc for {@link Mockito#when}
   *
   ...
   * </code></pre>
   *
   * If throwables contain a checked exception then it has to match one of the checked exceptions of
   * method signature.
   *
   * <p>You can specify throwables to be thrown for consecutive calls. In that case the last
   * throwable determines the behavior of further consecutive calls.
   *
   * <p>If throwable is null then exception will be thrown.
   *
   * <p>See examples in javadoc for {@link Mockito#when}
   *
   ...
   * </code></pre>
   *
   * <p>If the throwable class is a checked exception then it has to match one of the checked
   ...
   * <p>See examples in javadoc for {@link Mockito#when}
   *
   * <p>Note depending on the JVM, stack trace information may not be available in the generated
   * throwable instance. If you require stack trace information, use {@link
   ...
   * </code></pre>
   *
   * <p>Each throwable class will be instantiated for each method invocation.
   *
   * <p>If <code>throwableTypes</code> contain a checked exception then it has to match one of the
   * checked exceptions of method signature.
   *
   * <p>You can specify <code>throwableTypes</code> to be thrown for consecutive calls. In that case
   * the last throwable determines the behavior of further consecutive calls.
   *
   * <p>If throwable is null then exception will be thrown.
   *
   * <p>See examples in javadoc for {@link Mockito#when}
   *
   * <p>Note since JDK 7, invoking this method will raise a compiler warning "possible heap
   * pollution", this API is safe to use. If you don't want to see this warning it is possible to
   * chain {@link #thenThrow(Class)}
   *
   * <p>Note depending on the JVM, stack trace information may not be available in the generated
   * throwable instance. If you require stack trace information, use {@link
   * OngoingStubbing#thenThrow(Throwable...)} instead.
   ...
   *
   * <p>This method is an alias of {@link #thenAnswer(Answer)}. This alias allows more readable
   * tests on occasion, for example:
   *
   * <pre class="code"><code class="java">
   * //using 'then' alias:
   ...
   *
   * <p>It allows to create a stub in one line of code. This can be helpful to keep test code clean.
