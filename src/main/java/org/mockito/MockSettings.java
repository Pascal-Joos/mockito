...
public interface MockSettings extends Serializable {
   ...
   * corner cases.
   *
   * <p>This mysterious feature should be used very occasionally. The object under test should know
   * exactly its collaborators & dependencies. If you happen to use it often than please make sure
   * you are really producing simple, clean & readable code.
   *
   * <p>Examples:
   *
   * <pre class="code"><code class="java">
  ...
  /**
   * Specifies default answers to interactions. It's quite advanced feature and typically you don't
  ...
  /**
   * Configures the mock to be serializable with a specific serializable mode. With this feature you
   * can use a mock in a place that requires dependencies to be serializable.
   *
   * <p>WARNING: This should be rarely used in unit testing.
   *
   * <p>The behaviour was implemented for a specific use case of a BDD spec that had an unreliable
  ...
  /**
   * Enables real-time logging of method invocations on this mock. Can be used during test debugging
   * in order to find wrong interactions with this mock.
   *
  ...
  /**
   * Registers a listener for method invocations on this mock. The listener is notified every time a
   * method on this mock is called.
   *
   ...
   * no reason to supply multiple listeners but we wanted to keep the API simple and consistent with
   * {@link #invocationListeners(InvocationListener...)}.
   *
   * <p>Throws exception when any of the passed listeners is null or when the entire vararg array is
   * null.
   *
   ...
   * and its methods to mirror the mocked type as closely as possible. If this is not desired, this
   ...
   * Framework integrators can use this method to create instances of creation settings and use them
   * in advanced use cases, for example to create invocations with {@link InvocationFactory}, or to
   * implement custom {@link MockHandler}. Since {@link MockCreationSettings} is {@link
   * NotExtensible}, Mockito public API needs a creation method for this type.
   *
   ...
   * problems such as 'unnecessary stubbing' ({@link UnnecessaryStubbingException}) or for 'stubbing
   * argument mismatch' {@link PotentialStubbingProblem}.
   *
   * <pre class="code"><code class="java">
